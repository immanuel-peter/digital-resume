...
A Go control plane — a Cobra CLI, a Gin REST API, and a controller-runtime reconciler — that reads a single hostess.yml and deploys an entire multi-service stack to managed Kubernetes. Generates manifests with client-go, builds images via Docker / GCP Cloud Build / Azure ACR Tasks, runs managed Postgres on CloudNativePG, syncs secrets through Infisical, and issues TLS via cert-manager + Let's Encrypt; GKE and AKS infrastructure is provisioned with Pulumi, with a Next.js Studio dashboard for deploys, metrics, and logs.
